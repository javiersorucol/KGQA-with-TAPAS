...
from utils.Request_utils import translate, link_graph_elements , ask_tapas, get_entity_table, get_entity_triples, ask_gpt_v1, ask_gpt_v2, link_graph_elements_gpt_v1, get_entity_triples_lang_chain, get_entity_triples_graphdb, link_graph_entities_graphdb
...
@app.post(main_service.get('graphdb_endpoint'))
def ask_graphdb_with_gpt(question: QUERY_DTO):
   try:
      # Process the question to obtain linked elements (tranlation and linking service are involved)
      linked_elements = preprocess_question(question, linking_function=link_graph_entities_graphdb)
      ...
      res = get_entity_triples_graphdb(linked_elements, question.text)
